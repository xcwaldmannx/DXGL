...
void DXGLSwapChain::create(DXGLWindow* window) {
	m_window = window;
	...
	HRESULT result = DXGLMain::graphics()->factory()->CreateSwapChain(DXGLMain::graphics()->device(), &desc, &m_swapchain);
	...
	// temp blend state
	DXGLMain::graphics()->context()->OMSetBlendState(nullptr, nullptr, 0xFFFFFFFF);

	reload();
}

bool DXGLSwapChain::present(bool vsync) {
	m_swapchain->Present(vsync, 0);
	return true;
}

bool DXGLSwapChain::isPaused() {
	return m_isPaused;
}

void DXGLSwapChain::resize(unsigned int width, unsigned int height) {
	if (m_swapchain) {	
...
void DXGLSwapChain::setFullscreen(bool fullscreen) {
...
void DXGLSwapChain::createRenderTargetView(RESOURCE_VIEW_DESC* desc, ResourceViewSlot slot, SP_DXGLRenderTargetView* rtv) {
	...
	HRESULT result = DXGLMain::graphics()->device()->CreateRenderTargetView(buffer, nullptr, &tempRTV);
...
void DXGLSwapChain::createDepthStencilView(RESOURCE_VIEW_DESC* desc, ResourceViewSlot slot, SP_DXGLDepthStencilView* dsv) {
	ID3D11Texture2D* buffer = nullptr;
	...
	HRESULT result = DXGLMain::graphics()->device()->CreateDepthStencilView(buffer, nullptr, &tempDSV);
...
ID3D11Texture2D* DXGLSwapChain::createBuffer(RESOURCE_VIEW_DESC* desc, bool rtv) {
	...
	if (rtv) {
		...
		HRESULT result = DXGLMain::graphics()->device()->CreateTexture2D(&textureDesc, nullptr, &buffer);
		...
		HRESULT result = DXGLMain::graphics()->device()->CreateTexture2D(&depthStencilDesc, nullptr, &buffer);
...
SP_DXGLRenderTargetView DXGLSwapChain::getRTV(ResourceViewSlot slot) {
	return *m_rtvs[slot].target;
}

SP_DXGLDepthStencilView DXGLSwapChain::getDSV(ResourceViewSlot slot) {
	return *m_dsvs[slot].target;
}

SP_DXGLShaderResourceView DXGLSwapChain::getRenderToTexture(ResourceViewSlot slot) {
	...
	HRESULT result = DXGLMain::graphics()->device()->CreateShaderResourceView(resolvedBuffer, &srvDesc, &renderToTexture);
	...
	DXGLMain::graphics()->context()->ResolveSubresource(resolvedBuffer, 0, buffer, 0, format);
