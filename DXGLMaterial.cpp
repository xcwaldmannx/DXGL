...
DXGLMaterial::DXGLMaterial(const std::string& filepath, SP_DXGLGraphics graphics) {
	...
	HRESULT result = graphics->device()->CreateTexture2D(&texDesc, nullptr, &texture);
	...
	for (int i = 0; i < 5; i++) {
		...
		for (int j = 0; j < numMips; j++) {
			...
			graphics->context()->UpdateSubresource(texture, subresourceIndex, nullptr,
...
DXGLMaterial::DXGLMaterial(const MaterialData& data, SP_DXGLGraphics graphics) {
	...
	for (int i = 0; i < 5; i++) {
		if (m_materialIndices & (1 << i)) {
			for (int j = 0; j < numMips; j++) {
				...
				graphics->context()->UpdateSubresource(texture, subresourceIndex, nullptr,
					textures[i].data, rowPitch, rowPitch * mipHeight);
			}
		}
	}
...
DXGLMaterial::~DXGLMaterial() {
	if (m_srv) m_srv->Release();
}

ID3D11ShaderResourceView* DXGLMaterial::get() {
	return m_srv;
}
