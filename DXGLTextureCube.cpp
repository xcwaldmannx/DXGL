...
DXGLTextureCube::DXGLTextureCube(const std::string& filepath, SP_DXGLGraphics graphics) {
	...
	if (FAILED(result)) {
		throw std::exception("DXGLTextureCube texture could not be created.");
	}
	...
	if (FAILED(result)) {
		throw std::exception("DXGLTextureCube SRV could not be created.");
	}
...
DXGLTextureCube::~DXGLTextureCube() {
	if (m_srv) m_srv->Release();
}

ID3D11ShaderResourceView* DXGLTextureCube::get() {
	return m_srv;
}
